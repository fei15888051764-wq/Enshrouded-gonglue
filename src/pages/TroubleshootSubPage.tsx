...
import { Wrench, ChevronRight, Home, ArrowUp } from 'lucide-react';
import PageLayout from './PageLayout';
import { troubleshootSubSections } from '../data/troubleshootData';
import SectionGallery from '../components/SectionGallery';
import SubPageHero from '../components/SubPageHero';
import { troubleshootImages } from '../data/tipsUpdatesTroubleArmorImages';

interface TroubleshootSubPageProps {
  subId: string;
}
...
export default function TroubleshootSubPage({ subId }: TroubleshootSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('troubleshoot', id); };
  const section = troubleshootSubSections.find((s) => s.id === subId);
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Wrench className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          <p className="text-[var(--text-secondary)] text-sm mb-4">This troubleshooting topic could not be found.</p>
          <button onClick={() => navigate('troubleshoot')} className="game-btn px-4 py-2 text-xs">Back to Troubleshooting</button>
        </div>
      </PageLayout>
    );
  }

  const idx = troubleshootSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? troubleshootSubSections[idx - 1] : null;
  const next = idx < troubleshootSubSections.length - 1 ? troubleshootSubSections[idx + 1] : null;

  return (
    <PageLayout title={section.title} subtitle="Troubleshooting" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('troubleshoot')} className="hover:text-[var(--text-gold)] transition-colors">Troubleshooting</button>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[var(--text-gold)]">{section.title}</span>
        </div>
        <SubPageHero images={troubleshootImages[section.id]} />
        <div className="max-w-4xl">{section.content}</div>
        <SectionGallery images={troubleshootImages[section.id]} skipFirst />
