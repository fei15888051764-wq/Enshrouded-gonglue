...
import { Camera, ChevronRight, Home, ArrowUp } from 'lucide-react';
...
interface ScreenshotsSubPageProps {
  subId: string;
}

// Sub-navigation for screenshots sub-pages
...
export default function ScreenshotsSubPage({ subId }: ScreenshotsSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('screenshots', id); };

  const section = screenshotsSubSections.find((s) => s.id === subId);
  ...
  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Camera className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          <p className="text-[var(--text-secondary)] text-sm mb-4">This screenshot gallery could not be found.</p>
          <button onClick={() => navigate('screenshots')} className="game-btn px-4 py-2 text-xs">
            Back to Screenshots Home
          </button>
        </div>
      </PageLayout>
    );
  ...
  const idx = screenshotsSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? screenshotsSubSections[idx - 1] : null;
  const next = idx < screenshotsSubSections.length - 1 ? screenshotsSubSections[idx + 1] : null;
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('screenshots')} className="hover:text-[var(--text-gold)] transition-colors">
            Screenshots
          </button>
          <ChevronRight className="w-3 h-3" />
        ...
        {/* Content */}
        <SubPageHero images={screenshotsImages[section.id]} />
        <div className="max-w-4xl">
          {section.content}
        </div>
        <SectionGallery images={screenshotsImages[section.id]} skipFirst />
