...
export default function ItemsSubPage({ subId }: ItemsSubPageProps) {
  ...
  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Package className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          ...
          <button onClick={() => navigate('items')} className="game-btn px-4 py-2 text-xs">Back to Items & Builds</button>
        </div>
      </PageLayout>
    );
  }
    ...
    <PageLayout title={section.title} subtitle="Items & Builds" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('items')} className="hover:text-[var(--text-gold)] transition-colors">Items</button>
          <ChevronRight className="w-3 h-3" />
