    ...
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Wrench className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          ...
          <button onClick={() => navigate('crafting')} className="game-btn px-4 py-2 text-xs">Back to Crafting & Builds</button>
        </div>
      </PageLayout>
    );
  }
    ...
    <PageLayout title={section.title} subtitle="Crafting & Builds" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('crafting')} className="hover:text-[var(--text-gold)] transition-colors">Crafting</button>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[var(--text-gold)]">{section.title}</span>
        </div>
        <div className="max-w-4xl">{section.content}</div>
        <SectionGallery images={craftingImages[section.id]} />
