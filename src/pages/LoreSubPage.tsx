...
interface LoreSubPageProps {
  subId: string;
}

// Sub-navigation for lore sub-pages
...
export default function LoreSubPage({ subId }: LoreSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('lore', id); };

  const section = loreSubSections.find((s) => s.id === subId);
  ...
  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<BookOpen className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          <p className="text-[var(--text-secondary)] text-sm mb-4">This lore topic could not be found.</p>
          <button onClick={() => navigate('lore')} className="game-btn px-4 py-2 text-xs">
            Back to Lore Home
          </button>
        </div>
      </PageLayout>
  ...
  const idx = loreSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? loreSubSections[idx - 1] : null;
  const next = idx < loreSubSections.length - 1 ? loreSubSections[idx + 1] : null;

  return (
    <PageLayout
      title={section.title}
      subtitle="Game Lore & Story"
      icon={<>{section.icon}</>}
    >
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('lore')} className="hover:text-[var(--text-gold)] transition-colors">
            Lore
          </button>
          <ChevronRight className="w-3 h-3" />
