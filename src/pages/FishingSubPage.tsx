...
export default function FishingSubPage({ subId }: FishingSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('fishing', id); };
  const section = fishingSubSections.find((s) => s.id === subId);
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Anchor className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          <p className="text-[var(--text-secondary)] text-sm mb-4">This fishing topic could not be found.</p>
          <button onClick={() => navigate('fishing')} className="game-btn px-4 py-2 text-xs">Back to Fishing System</button>
        </div>
      </PageLayout>
    );
  }

  const idx = fishingSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? fishingSubSections[idx - 1] : null;
  const next = idx < fishingSubSections.length - 1 ? fishingSubSections[idx + 1] : null;

  return (
    <PageLayout title={section.title} subtitle="Fishing System" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('fishing')} className="hover:text-[var(--text-gold)] transition-colors">Fishing</button>
          <ChevronRight className="w-3 h-3" />
