...
export default function WalkthroughHomePage() {
  const { navigate } = usePage();

  return (
    <PageLayout
      title="Walkthrough"
      subtitle="9-part complete walkthrough — from Cinder Vault to Flame Level 9"
    ...
    >
      <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)]">
        ...
        <span className="text-[var(--text-gold)]">Walkthrough</span>
      </div>

      <div className="game-panel corner-decor p-6 mb-8">
        <p className="text-[var(--text-secondary)] text-sm leading-relaxed">
          Complete walkthrough guide for Enshrouded. Explore all subsections below.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {walkthroughSubSections.map((section) => (
          <button
            key={section.id}
            onClick={() => navigate('walkthrough', section.id)}
