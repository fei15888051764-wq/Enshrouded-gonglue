...
interface WalkthroughSubPageProps {
  subId: string;
}
...
export default function WalkthroughSubPage({ subId }: WalkthroughSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('walkthrough', id); };
  const section = walkthroughSubSections.find((s) => s.id === subId);
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Map className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          ...
          <button onClick={() => navigate('walkthrough')} className="game-btn px-4 py-2 text-xs">Back to Walkthrough & Builds</button>
        </div>
      </PageLayout>
    );
  }

  const idx = walkthroughSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? walkthroughSubSections[idx - 1] : null;
  const next = idx < walkthroughSubSections.length - 1 ? walkthroughSubSections[idx + 1] : null;

  return (
    <PageLayout title={section.title} subtitle="Walkthrough & Builds" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('walkthrough')} className="hover:text-[var(--text-gold)] transition-colors">Walkthrough</button>
          <ChevronRight className="w-3 h-3" />
