...
import { ChevronRight, Home as HomeIcon, ArrowUp } from 'lucide-react';
import PageLayout from './PageLayout';
import SectionGallery from '../components/SectionGallery';
import SubPageHero from '../components/SubPageHero';
import { baseImages } from '../data/baseCraftingItemsImages';
import { baseSubSections } from '../data/baseData';

interface BaseSubPageProps {
  subId: string;
}
...
export default function BaseSubPage({ subId }: BaseSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('base', id); };
  const section = baseSubSections.find((s) => s.id === subId);
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<HomeIcon className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          ...
          <button onClick={() => navigate('base')} className="game-btn px-4 py-2 text-xs">Back to Base & Builds</button>
        </div>
      </PageLayout>
    );
  }

  const idx = baseSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? baseSubSections[idx - 1] : null;
  const next = idx < baseSubSections.length - 1 ? baseSubSections[idx + 1] : null;

  return (
    <PageLayout title={section.title} subtitle="Base & Builds" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          <button onClick={() => navigate('home')} className="flex items-center gap-1 hover:text-[var(--text-gold)] transition-colors">
            <HomeIcon className="w-3 h-3" /><span>Home</span>
          </button>
          <ChevronRight className="w-3 h-3" />
          <button onClick={() => navigate('base')} className="hover:text-[var(--text-gold)] transition-colors">Base</button>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[var(--text-gold)]">{section.title}</span>
        </div>
        <SubPageHero images={baseImages[section.id]} />
        <div className="max-w-4xl">{section.content}</div>
        <SectionGallery images={baseImages[section.id]} skipFirst />
