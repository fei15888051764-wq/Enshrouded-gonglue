...
export default function TroubleshootHomePage() {
  ...
  return (
    <PageLayout
      ...
      subtitle="Fix crashes, lag, connection issues, and technical problems"
    ...
    >
      <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)]">
        ...
        <span className="text-[var(--text-gold)]">Troubleshooting</span>
      </div>

      <div className="game-panel corner-decor p-6 mb-8">
        <p className="text-[var(--text-secondary)] text-sm leading-relaxed">
          Complete troubleshooting guide for Enshrouded. Find solutions for crashes, performance issues, multiplayer problems, save data recovery, and more.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {troubleshootSubSections.map((section) => (
          <button
            key={section.id}
            onClick={() => navigate('troubleshoot', section.id)}
