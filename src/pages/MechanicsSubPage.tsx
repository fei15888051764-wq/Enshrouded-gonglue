...
interface MechanicsSubPageProps {
  subId: string;
}
...
export default function MechanicsSubPage({ subId }: MechanicsSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('mechanics', id); };
  const section = mechanicsSubSections.find((s) => s.id === subId);
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Settings className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          ...
          <button onClick={() => navigate('mechanics')} className="game-btn px-4 py-2 text-xs">Back to Mechanics & Builds</button>
        </div>
      </PageLayout>
    );
  }

  const idx = mechanicsSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? mechanicsSubSections[idx - 1] : null;
  const next = idx < mechanicsSubSections.length - 1 ? mechanicsSubSections[idx + 1] : null;

  return (
    <PageLayout title={section.title} subtitle="Mechanics & Builds" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('mechanics')} className="hover:text-[var(--text-gold)] transition-colors">Mechanics</button>
          <ChevronRight className="w-3 h-3" />
