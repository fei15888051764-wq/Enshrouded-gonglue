...
export default function MapSubPage({ subId }: MapSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('map', id); };
  const section = mapSubSections.find((s) => s.id === subId);
    ...
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Map className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          <p className="text-[var(--text-secondary)] text-sm mb-4">This map topic could not be found.</p>
          <button onClick={() => navigate('map')} className="game-btn px-4 py-2 text-xs">Back to Map</button>
        </div>
      </PageLayout>
    );
  }

  const idx = mapSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? mapSubSections[idx - 1] : null;
  const next = idx < mapSubSections.length - 1 ? mapSubSections[idx + 1] : null;

  return (
    <PageLayout title={section.title} subtitle="Map & Locations" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('map')} className="hover:text-[var(--text-gold)] transition-colors">Map</button>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[var(--text-gold)]">{section.title}</span>
        </div>
        <div className="max-w-4xl">{section.content}</div>
        <SectionGallery images={mapImages[section.id]} />
