...
import { Lightbulb, ChevronRight, Home, ArrowUp } from 'lucide-react';
...
export default function TipsSubPage({ subId }: TipsSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('tips', id); };
  const section = tipsSubSections.find((s) => s.id === subId);
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Lightbulb className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          ...
          <button onClick={() => navigate('tips')} className="game-btn px-4 py-2 text-xs">Back to Tips & Builds</button>
        </div>
      </PageLayout>
    );
  }

  const idx = tipsSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? tipsSubSections[idx - 1] : null;
  const next = idx < tipsSubSections.length - 1 ? tipsSubSections[idx + 1] : null;

  return (
    <PageLayout title={section.title} subtitle="Tips & Builds" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('tips')} className="hover:text-[var(--text-gold)] transition-colors">Tips</button>
          <ChevronRight className="w-3 h-3" />
