...
export default function QuestsSubPage({ subId }: QuestsSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('quests', id); };

  const section = questsSubSections.find((s) => s.id === subId);
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<ScrollText className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          <p className="text-[var(--text-secondary)] text-sm mb-4">This quest topic could not be found.</p>
          <button onClick={() => navigate('quests')} className="game-btn px-4 py-2 text-xs">
            Back to Quests & Missions
          </button>
        </div>
      </PageLayout>
    );
  ...
  const idx = questsSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? questsSubSections[idx - 1] : null;
  const next = idx < questsSubSections.length - 1 ? questsSubSections[idx + 1] : null;

  return (
    <PageLayout
      title={section.title}
      subtitle="Quests & Missions"
      icon={<>{section.icon}</>}
    >
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('quests')} className="hover:text-[var(--text-gold)] transition-colors">
            Quests
          </button>
          <ChevronRight className="w-3 h-3" />
