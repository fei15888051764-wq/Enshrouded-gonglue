...
import { Swords, ChevronRight, Home, ArrowUp } from 'lucide-react';
...
export default function SkillsSubPage({ subId }: SkillsSubPageProps) {
  ...
  const handleNav = (id: string) => { navigate('skills', id); };
  const section = skillsSubSections.find((s) => s.id === subId);
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  if (!section) {
    return (
      <PageLayout title="Not Found" subtitle="" icon={<Swords className="w-6 h-6 text-[var(--text-gold)]" />}>
        <div className="game-panel corner-decor p-6 text-center">
          ...
          <button onClick={() => navigate('skills')} className="game-btn px-4 py-2 text-xs">Back to Skills & Builds</button>
        </div>
      </PageLayout>
    );
  }

  const idx = skillsSubSections.findIndex((s) => s.id === subId);
  const prev = idx > 0 ? skillsSubSections[idx - 1] : null;
  const next = idx < skillsSubSections.length - 1 ? skillsSubSections[idx + 1] : null;

  return (
    <PageLayout title={section.title} subtitle="Skills & Builds" icon={<>{section.icon}</>}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)] flex-wrap">
          ...
          <button onClick={() => navigate('skills')} className="hover:text-[var(--text-gold)] transition-colors">Skills</button>
          <ChevronRight className="w-3 h-3" />
