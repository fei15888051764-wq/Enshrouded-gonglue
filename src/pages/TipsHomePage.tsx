...
export default function TipsHomePage() {
  ...
  return (
    <PageLayout
      title="Tips & Tricks"
      subtitle="Complete Tips & Tricks Guide — Beginner to Advanced"
      icon={<Lightbulb className="w-6 h-6 text-[var(--text-gold)]" />}
    >
      <div className="flex items-center gap-2 mb-6 text-xs text-[var(--text-muted)]">
        ...
        <span className="text-[var(--text-gold)]">Tips & Tricks</span>
      </div>

      <div className="game-panel corner-decor p-6 mb-8">
        <p className="text-[var(--text-secondary)] text-sm leading-relaxed">
          Complete tips & tricks guide for Enshrouded. Explore all subsections below.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {tipsSubSections.map((section) => (
          <button
            key={section.id}
            onClick={() => navigate('tips', section.id)}
